...
Identity: `COALESCE(user_id, event_params.user_id, user_pseudo_id)`  
...
|---|-----|----------------------|-------------------|-------------|
| 1 | DAU | `product/01_dau.sql` ← `v_daily_active_users` | `raw/01_dau.sql` or signals `dau` | Any event that day |
