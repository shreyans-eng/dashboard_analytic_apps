...
| 6 | D1 / D4 / D7 | `d1_retention_rate` / `d4` / `d7` / `d4_d7` | Returned = opened the app (`session_start` / `App_open` / `first_open`) on D+1 / D+4 / D+7; D4–D7 = any of days 4–7. Push is not a return. Cohort = `first_open` |
| 7 | Scans / user | `scans_per_dau` + `scans_p10`…`scans_p99` | Mean = success events ÷ DAU. Percentiles = successful IDs per **DAU** user-day including 0 (P10, P25, P50, P75, **P90**, P95, P99). `scans_per_scanning_user` is the scanners-only mean |
...
| Installs + time used | `first_open` devices; “went in” = ≥10s (`engagement_time_msec` or `session_length_seconds`). Time P10–P99 among all installs, including 0s |
| D0 / D1 percentiles | Install cohort. D0 went-in + D1 opened (DAU events). Time and scans P10–P99 include zeros (D0 = all installs; D1 = openers) |
