-- =============================================================================
-- Coinzy MVP #8 — Identify funnel (detailed steps + drop-off)
-- Events from CoinzyAndroid (verified):
--   CameraScreen.kt, IdentifyViewModel.kt, CoinAnalysisScreen.kt,
--   CoinMatchScreen.kt, HomeScreen.kt, BottomNavigationBar.kt, FreeScanLimitUtil.kt
...
step_users AS (
  ...
  SELECT '04c_permission_denied', 'Camera permission DENIED (drop)', 7,
    ...
    COUNTIF(event_name_base = 'camera_permission_denied')
  FROM base
  UNION ALL
  ...
  FROM base
  UNION ALL
  SELECT '06_crop_screen', 'Crop screen opened', 9,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
  ...
  FROM base
  UNION ALL
  SELECT '07_crop_confirm', 'Crop confirmed (tick)', 10,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
  ...
  FROM base
  UNION ALL
  SELECT '08_submit', 'Photos submitted', 11,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
  ...
  FROM base
  UNION ALL
  SELECT '09_quota_block', 'Quota / limit block (drop)', 12,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
      'Identified_limit_reached', 'free_scan_limit_exceeded', 'free_scan_blocked',
      'free_scan_success_quota_exhausted', 'Identification_unsuccessful_limit_reached',
  ...
  FROM base
  UNION ALL
  SELECT '10_success', 'Identification SUCCESS', 13,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
  ...
  FROM base
  UNION ALL
  SELECT '10b_failure', 'Identification FAILURE (drop)', 14,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
  ...
  FROM base
  UNION ALL
  SELECT '12_option_chosen', 'Option chosen (typo event name in app)', 16,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
  ...
  FROM base
  UNION ALL
  SELECT '13_details', 'ID details / coin details after ID', 17,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
