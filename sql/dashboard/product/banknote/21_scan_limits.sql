-- =============================================================================
-- Banknote scan limits: free vs subscribed
-- Paid: Subs_confirm (plus store IAP / lifetime as fallback)
-- Quota: identiifcation_limit_exceeded (app typo) and Identified_limit_reached
-- =============================================================================
  ...
  WHERE _TABLE_SUFFIX BETWEEN paid_from_s AND end_s
    ...
    AND (
      REGEXP_REPLACE(event_name, r'_(android|ios)$', '') IN (
        'Subs_confirm', 'in_app_purchase', 'life_time_access'
      )
      OR (
