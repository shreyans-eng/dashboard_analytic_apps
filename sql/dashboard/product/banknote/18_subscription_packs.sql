...
-- Pack name is on Subs_pack (pack_name + discounted/non-discounted), not on
-- Subs_confirm. Attribute each person to that day's last named pack click
-- when the confirm event has no pack_name.
...
pack_clicks AS (
  ...
  WHERE uid IS NOT NULL
    AND event_name_base IN ('Subs_pack', 'subs_pack')
