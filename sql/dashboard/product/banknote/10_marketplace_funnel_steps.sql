-- =============================================================================
-- Banknote MVP #10 — Marketplace + Feed funnel (detailed steps + drop-off)
...
step_users AS (
  ...
  SELECT 'M06_sell_cta', 'Marketplace: add-for-sale CTA', 6,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
      'add_for_sale_button_marketplace', 'add_for_sale_details_scr_btn'
  ...
  SELECT 'M07_listing_created', 'Marketplace: listing published', 7,
    COUNT(DISTINCT CASE WHEN event_name_base IN ('market_add', 'market_contact_Add_for_sale') THEN resolved_user_id END),
    COUNTIF(event_name_base IN ('market_add', 'market_contact_Add_for_sale'))
  FROM base
  UNION ALL
  ...
  SELECT 'F04_post', 'Feed: create post', 12,
    COUNT(DISTINCT CASE WHEN event_name_base IN ('feed_add', 'feed_post') THEN resolved_user_id END),
    COUNTIF(event_name_base IN ('feed_add', 'feed_post'))
  FROM base
  UNION ALL
  -- Combined KPI engagement (screen opens)
