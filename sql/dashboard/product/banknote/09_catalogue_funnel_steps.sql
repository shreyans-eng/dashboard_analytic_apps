...
step_users AS (
  SELECT '01_collection_tab' AS step_id, 'Entry: Collection bottom nav' AS step_label, 1 AS step_order,
    COUNT(DISTINCT CASE WHEN event_name_base = 'private_collection_bottom_nav' THEN resolved_user_id END) AS users,
    COUNTIF(event_name_base = 'private_collection_bottom_nav') AS events
  FROM base
  UNION ALL
  ...
  FROM base
  UNION ALL
  SELECT '06_details_collection', 'Banknote details (from collection)', 6,
    COUNT(DISTINCT CASE WHEN event_name_base = 'banknote_details_collection' THEN resolved_user_id END),
    COUNTIF(event_name_base = 'banknote_details_collection')
  FROM base
  UNION ALL
  ...
  SELECT '08_global_cta', 'Entry: Global catalogue CTA (Home)', 8,
    COUNT(DISTINCT CASE WHEN event_name_base IN ('Global_catalogue', 'View_all_button_global') THEN resolved_user_id END),
    COUNTIF(event_name_base IN ('Global_catalogue', 'View_all_button_global'))
  FROM base
  UNION ALL
  ...
  FROM base
  UNION ALL
  SELECT '11_details_global', 'Banknote details (from global)', 11,
    COUNT(DISTINCT CASE WHEN event_name_base = 'banknote_details_global' THEN resolved_user_id END),
    COUNTIF(event_name_base = 'banknote_details_global')
  FROM base
  UNION ALL
  SELECT '12_filter_global', 'Filter (global catalogue)', 12,
    COUNT(DISTINCT CASE WHEN event_name_base IN (
      'Filter_button_global', 'filter_field_selected_catalogue', 'Predefined_filter_global'
    ) THEN resolved_user_id END),
    COUNTIF(event_name_base IN (
      'Filter_button_global', 'filter_field_selected_catalogue', 'Predefined_filter_global'
    ))
  ...
  SELECT '14_wishlist_add', 'Added to wishlist', 14,
    COUNT(DISTINCT CASE WHEN event_name_base = 'Added_to_collection_wishlist' THEN resolved_user_id END),
    COUNTIF(event_name_base = 'Added_to_collection_wishlist')
  FROM base
  UNION ALL
