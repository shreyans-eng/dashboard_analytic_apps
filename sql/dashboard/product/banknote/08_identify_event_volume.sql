-- =============================================================================
-- Banknote MVP #8 — Identify: event volume by screen/action (which fires most)
-- Use to see which sub-screens users open most within Identify.
-- =============================================================================
...
labeled AS (
  SELECT
    ...
    CASE
      ...
      WHEN event_name_base IN ('Identification_screen', 'photo_screen', 'camera_opened') THEN '02_camera_screen'
      ...
      ) THEN '04_capture_upload'
      WHEN event_name_base IN (
        'photo_cropping_screen_1', 'photo_cropping_screen_2',
        'photo_crop_tick_1', 'photo_crop_tick_2'
      ) THEN '05_crop'
      WHEN event_name_base IN ('photo_submit_button', 'photos_submitted') THEN '06_submit'
      WHEN event_name_base IN (
        'Identified_limit_reached', 'identiifcation_limit_exceeded',
        'scan_quota_exhausted', 'scan_quota_info_viewed'
      ) THEN '07_quota'
      WHEN event_name_base IN (
        'identification_done_success', 'Identification_done_success',
        'Identification_done', 'scan_classified'
      ) THEN '08_success'
      WHEN event_name_base IN (
      ...
      WHEN event_name_base IN (
        'identification_top5_matches', 'idetnification_option_chosen',
      ...
      WHEN STARTS_WITH(event_name_base, 'Added_to_collection') THEN '11_add_collection'
      ELSE NULL
    END AS funnel_stage
  FROM base
)
