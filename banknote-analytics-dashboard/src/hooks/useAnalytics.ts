...
function dashboardQueryVersion(name: string) {
  ...
  if (name === 'mvp-scans-per-user') return 'v8';
  ...
  if (name === 'mvp-paywall') return 'v2';
  if (name === 'subscription-packs' || name === 'subscription-tiers') return 'v3';
  return 'v4';
}
...
export function useDashboardMetric(name: string, params: QueryParams, enabled = true) {
  ...
  const product =
    ...
    || (name === 'subscription-packs' && productId === 'compare')
...
export function useCompareSubscriptions(params: QueryParams, enabled = true) {
  ...
  return useQuery({
    queryKey: queryKey('dashboard:compare-subscriptions:v3', p),
...
export function useSubscriptionPacks(params: QueryParams, enabled = true) {
  const { productId } = useProduct();
  const p = withProduct(params, productId);
  return useQuery({
    queryKey: queryKey('dashboard:subscription-packs:v3', p),
...
export function useFunnel(funnelId: string, params: QueryParams, enabled = true) {
  ...
  const p = withProduct(params, productId);
  return useQuery({
    queryKey: queryKey(`funnel:${funnelId}:v21`, p),
