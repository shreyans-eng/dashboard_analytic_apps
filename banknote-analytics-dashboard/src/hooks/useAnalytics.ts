...
function dashboardQueryVersion(name: string) {
  ...
  if (name === 'subscription-packs' || name === 'subscription-tiers') return 'v12';
  return 'v4';
}
...
export function useCompareSubscriptions(params: QueryParams, enabled = true) {
  ...
  return useQuery({
    queryKey: queryKey('dashboard:compare-subscriptions:v12', p),
...
export function useSubscriptionPacks(params: QueryParams, enabled = true) {
  ...
  return useQuery({
    queryKey: queryKey('dashboard:subscription-packs:v12', p),
